CREATE TABLE public.documents (
  ...
  visibility text NOT NULL DEFAULT 'private' CHECK (visibility IN ('private', 'group', 'review', 'college')),
  subject text,
...
CREATE UNIQUE INDEX documents_user_hash_unique ON public.documents(user_id, content_hash) WHERE content_hash IS NOT NULL;
...
CREATE TABLE public.practice_sessions (
  ...
  source_id uuid,
  session_type text NOT NULL DEFAULT 'technical' CHECK (session_type IN ('technical', 'coding', 'behavioral')),
...
CREATE TRIGGER trg_job_descriptions_updated BEFORE UPDATE ON public.job_descriptions FOR EACH ROW EXECUTE FUNCTION public.tg_set_updated_at();

GRANT SELECT ON public.documents, public.document_concepts, public.technical_questions, public.practice_sessions, public.practice_attempts, public.job_descriptions, public.coding_submissions, public.feedback, public.usage_events TO authenticated;
GRANT ALL ON public.documents, public.document_concepts, public.technical_questions, public.practice_sessions, public.practice_attempts, public.job_descriptions, public.coding_submissions, public.feedback, public.usage_events, public.coding_problems TO service_role;
...
BEGIN
  FOREACH table_name IN ARRAY ARRAY[
    ...
    EXECUTE format('CREATE POLICY %I ON public.%I FOR SELECT TO authenticated USING (auth.uid() = user_id)', 'own_' || table_name || '_select', table_name);
  END LOOP;
END $$;

ALTER TABLE public.coding_problems ENABLE ROW LEVEL SECURITY;

CREATE POLICY study_materials_owner_insert ON storage.objects FOR INSERT TO authenticated
  WITH CHECK (bucket_id = 'study-materials' AND (storage.foldername(name))[1] = auth.uid()::text);
CREATE POLICY study_materials_owner_select ON storage.objects FOR SELECT TO authenticated
  USING (bucket_id = 'study-materials' AND (storage.foldername(name))[1] = auth.uid()::text);
CREATE POLICY study_materials_owner_delete ON storage.objects FOR DELETE TO authenticated
  USING (bucket_id = 'study-materials' AND (storage.foldername(name))[1] = auth.uid()::text);
...
BEGIN
  ...
  DELETE FROM public.request_rate_limits WHERE user_id = p_user_id AND window_started_at < clock_timestamp() - interval '2 days';
...
INSERT INTO public.coding_problems (slug, title, statement, difficulty, topics, constraints, examples, starter_code, hidden_tests, explanation)
VALUES
('two-sum', 'Two Sum',
...
('binary-search', 'Binary Search',
 ...
 '{"javascript":"function search(nums, target) {\n  // return index\n}","python":"def search(nums, target):\n    # return index\n    pass","c":"int search(int* nums, int numsSize, int target) {\n    return -1;\n}","cpp":"#include <vector>\nusing namespace std;\n\nint search(vector<int>& nums, int target) {\n    return -1;\n}","csharp":"public class Solution {\n    public int Search(int[] nums, int target) {\n        return -1;\n    }\n}"}',
