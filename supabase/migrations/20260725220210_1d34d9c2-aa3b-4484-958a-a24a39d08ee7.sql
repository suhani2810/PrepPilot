...
ALTER TABLE public.interviews
  ADD CONSTRAINT interviews_duration_range
    CHECK (duration_minutes BETWEEN 5 AND 180) NOT VALID,
  ADD CONSTRAINT interviews_status_allowed
    CHECK (status IN ('active', 'completed')) NOT VALID,
...
ALTER TABLE public.interview_messages
  ADD CONSTRAINT interview_messages_role_allowed
    CHECK (role IN ('ai', 'user')) NOT VALID,
  ADD CONSTRAINT interview_messages_difficulty_range
    CHECK (difficulty IS NULL OR difficulty BETWEEN 1 AND 5) NOT VALID,
  ADD CONSTRAINT interview_messages_order_nonnegative
...
BEGIN
  ...
  CASE p_action
    WHEN 'transcribe' THEN
      v_limit := 12;
      v_window_seconds := 60;
    WHEN 'parse_resume' THEN
      v_limit := 5;
      v_window_seconds := 3600;
    WHEN 'start_interview' THEN
      v_limit := 10;
      v_window_seconds := 3600;
    WHEN 'submit_answer' THEN
      v_limit := 60;
