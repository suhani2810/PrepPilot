...
export function PrepPilotMark({ className, size = 28 }: { className?: string; size?: number }) {
  return (
    ...
    >
      <defs>
        <linearGradient id="pp-mark-grad" x1="4" y1="4" x2="28" y2="28" gradientUnits="userSpaceOnUse">
      ...
      {/* horizon */}
      <path d="M5 18 Q 16 14 27 18" stroke="url(#pp-mark-grad)" strokeWidth="1.5" strokeLinecap="round" opacity="0.55" />
      {/* ascending vector */}
      <path
...
export function PrepPilotLogo({ className, compact = false }: { className?: string; compact?: boolean }) {
  return (
    <div className={cn("flex items-center gap-2", className)}>
      <PrepPilotMark size={compact ? 24 : 28} />
      <span className={cn("font-sans font-semibold leading-none tracking-tight text-foreground", compact ? "text-[1.05rem]" : "text-[1.2rem]")}>
