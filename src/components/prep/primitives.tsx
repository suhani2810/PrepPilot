...
export function SectionEyebrow({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <span className={cn("inline-flex items-center gap-2 text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground", className)}>
...
export function SectionHeader({
  ...
  return (
    <div className={cn("max-w-2xl", align === "center" && "mx-auto text-center")}>
      ...
      <h2 className="mt-3 font-display text-3xl font-semibold tracking-tight sm:text-4xl md:text-[2.6rem]">{title}</h2>
...
}: { children: ReactNode; className?: string; elevated?: boolean; interactive?: boolean }) {
  return (
    <div
      className={cn(
...
}: { label: string; value: ReactNode; hint?: ReactNode; icon?: ReactNode; trend?: "up" | "down" | "flat" }) {
  return (
    <Surface elevated className="p-5">
        ...
        {trend && (
          <span
            className={cn(
              ...
              trend === "up" && "bg-[color-mix(in_oklab,var(--success)_15%,transparent)] text-[color:var(--success)]",
      ...
      <p className="mt-3 font-display text-[2.25rem] font-semibold leading-none tabular-nums">{value}</p>
...
export function EmptyState({
  ...
  return (
    <Surface className={cn("flex flex-col items-center justify-center gap-3 border-dashed py-12 px-6 text-center", className)}>
      {icon && (
...
export function ReadinessGauge({
  value, size = 180, label = "Readiness",
  ...
  return (
    <div className="relative inline-flex flex-col items-center" style={{ width: size, height: size }}>
      <svg width={size} height={size} className="-rotate-90">
        <defs>
          <linearGradient id="gauge-grad" x1="0" y1="0" x2={size} y2={size} gradientUnits="userSpaceOnUse">
        ...
        <circle
          ...
          stroke="url(#gauge-grad)" strokeWidth="8" fill="none"
          strokeLinecap="round"
          strokeDasharray={c}
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className="font-display text-4xl font-semibold tabular-nums text-gradient">{Math.round(v)}<span className="text-lg text-muted-foreground">%</span></span>
        <span className="mt-1 text-[10px] font-medium uppercase tracking-[0.18em] text-muted-foreground">{label}</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      <div className="flex items-baseline justify-between text-sm">
        ...
        <span className="font-medium tabular-nums">{value.toFixed(1)}<span className="text-xs text-muted-foreground">/10</span></span>
      </div>
