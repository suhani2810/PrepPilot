  ...
  head: () => ({
    meta: [
      { title: "Dashboard — PrepPilot" },
      {
        name: "description",
        content:
          "Your interview readiness command center — track scores, resume interviews, and see what to focus on next.",
      },
    ],
  }),
  component: Dashboard,
...
function Dashboard() {
    ...
    (async () => {
      ...
      const [{ data: ivs }, { data: p }, { data: cp }] = await Promise.all([
        supabase
          .from("interviews")
          .select(
            "id,role,status,overall_score,readiness_score,started_at,completed_at,final_report",
          )
          .eq("user_id", u.user.id)
          .order("started_at", { ascending: false }),
        supabase
          .from("profiles")
          .select("display_name, readiness_score")
          .eq("id", u.user.id)
          .maybeSingle(),
  ...
  const scored = interviews.filter((i) => i.status === "completed" && i.overall_score != null);
  const avg = scored.length
    ? Math.round((scored.reduce((a, i) => a + Number(i.overall_score), 0) / scored.length) * 10) /
      10
    : null;
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                PrepPilot uses it to build every question.
              </p>
            </div>
          </div>
          <Link to="/resume">
        ...
        <Surface className="mt-6 flex flex-wrap items-center justify-between gap-4 border-primary/40 p-5">
          ...
          <Link to="/interview/$interviewId" params={{ interviewId: active.id }}>
            <Button className="bg-gradient-primary text-primary-foreground">
              Resume session <ArrowRight className="ml-2 h-3.5 w-3.5" />
            </Button>
          </Link>
        </Surface>
      )}
          ...
          <div className="relative grid gap-8 md:grid-cols-[1.5fr_1fr] md:items-center">
            <div>
              ...
              <p className="mt-3 max-w-lg text-sm leading-relaxed text-muted-foreground">
                Readiness %, performance trend, strengths, and focus areas all appear here after
                your first scored session. Nothing is precomputed and nothing is faked — every
                number comes from an interview you actually took.
              </p>
              ...
              <ul className="mt-6 grid gap-2 text-sm text-muted-foreground sm:grid-cols-2">
                <li className="flex items-start gap-2">
                  <TrendingUp className="mt-0.5 h-3.5 w-3.5 text-primary" /> Performance trend
                  across sessions
                </li>
                <li className="flex items-start gap-2">
                  <Trophy className="mt-0.5 h-3.5 w-3.5 text-primary" /> Readiness % from real
                  evaluations
                </li>
                <li className="flex items-start gap-2">
                  <Compass className="mt-0.5 h-3.5 w-3.5 text-primary" /> Personalized focus areas
                </li>
                <li className="flex items-start gap-2">
                  <Sparkles className="mt-0.5 h-3.5 w-3.5 text-primary" /> Post-interview learning
                  roadmap
                </li>
              </ul>
            </div>
          ...
          <div className="mt-6 grid gap-4 lg:grid-cols-[1.25fr_1fr]">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
              <Stat
                ...
                hint={`${interviews.length} total session${interviews.length === 1 ? "" : "s"}`}
              />
              <Stat
                label="Average score"
                value={avg != null ? `${avg}` : "—"}
                icon={<TrendingUp className="h-3.5 w-3.5" />}
                hint="out of 10 across scored sessions"
              />
            </div>
          </div>
            ...
            <Surface className="p-6">
              ...
              {trend.length >= 2 ? (
                ...
                <EmptyState
                  ...
                  description={
                    trend.length === 1
                      ? "One more session and your trend line comes to life."
                      : "Complete a couple of interviews to see how you're progressing."
                  }
                  action={
                    <Link to="/interview/new">
            ...
            <Surface className="p-6">
              ...
              {scored.length === 0 ? (
                <p className="mt-4 text-sm text-muted-foreground">
                  Complete a session to see your strongest areas and what to work on.
                </p>
              ) : (
                <div className="mt-4 space-y-5">
                  <FocusList
                    ...
                    tone="success"
                    items={strengthTally}
                    empty="Strengths will surface after a full session."
                  />
                  <FocusList
          ...
          {interviews.length === 0 ? (
            <EmptyState
              ...
              action={
                <Link to="/interview/new">
                  <Button size="sm" className="bg-gradient-primary text-primary-foreground">
                    Start your first interview <ArrowRight className="ml-2 h-3.5 w-3.5" />
                  </Button>
                </Link>
              }
            />
              ...
              {interviews.slice(0, 5).map((i) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 sm:gap-3">
                    <Badge
                      variant={i.status === "completed" ? "outline" : "secondary"}
        ...
        <ul className="mt-2 space-y-1.5">
          {items.map((it) => (
            <li key={it.label} className="flex items-start gap-2 text-sm">
              <span
                className={`mt-1.5 h-1.5 w-1.5 flex-none rounded-full ${tone === "success" ? "bg-[color:var(--success)]" : "bg-highlight"}`}
              />
