...
import { CheckCircle2, Sparkles, AlertTriangle, ArrowRight, Target, MessageSquareQuote } from "lucide-react";
import { DimensionBar, EmptyState, ReadinessGauge, SectionEyebrow, Skeleton, Stat, Surface } from "@/components/prep/primitives";

export const Route = createFileRoute("/_authenticated/interview/$interviewId/report")({
  head: () => ({
    meta: [
      { title: "Interview Report — PrepPilot" },
      { name: "description", content: "Detailed post-interview report with dimension scores, strengths, gaps, and ideal answers." },
    ],
  }),
  component: Report,
});
...
type Interview = {
  role: string; overall_score: number | null; readiness_score: number | null; completed_at: string | null;
  final_report: {
...
type Msg = { id: string; role: "ai" | "user"; content: string; order_index: number; topic: string | null; difficulty: number | null };
type Ev = {
  interview_message_id: string; overall_score: number | null;
  strengths: string[] | null; missing_concepts: string[] | null;
  ideal_answer: string | null; weaknesses: string[] | null;
};
...
function Report() {
  ...
  useEffect(() => {
    (async () => {
      const [{ data: i }, { data: m }, { data: e }] = await Promise.all([
        supabase.from("interviews").select("role, overall_score, readiness_score, final_report, completed_at").eq("id", interviewId).single(),
        supabase.from("interview_messages").select("id,role,content,order_index,topic,difficulty").eq("interview_id", interviewId).order("order_index"),
        supabase.from("evaluations").select("interview_message_id, overall_score, strengths, weaknesses, missing_concepts, ideal_answer").eq("interview_id", interviewId),
      ]);
  ...
  if (!iv) return <div className="mx-auto max-w-4xl px-6 py-10 text-muted-foreground">Report not found.</div>;
  ...
  const radarData = Object.entries(dims).map(([k, v]) => ({ name: DIM_LABELS[k] ?? k, value: Number(v) || 0 }));
        ...
        <EmptyState
          ...
          action={
            <div className="flex justify-center gap-2">
              ...
              <Link to="/interview/new"><Button className="bg-gradient-primary text-primary-foreground">Start a new interview</Button></Link>
            </div>
          }
        />
      </div>
  ...
  const weakDims = Object.entries(dims).sort((a, b) => Number(a[1]) - Number(b[1])).slice(0, 2)
    .map(([k]) => DIM_LABELS[k] ?? k);
  const nextSteps = [
    r?.weaknesses?.length ? `Practice targeted questions around: ${r.weaknesses.slice(0, 3).join(", ")}.` : null,
        ...
        <div className="grid gap-8 md:grid-cols-[auto_1fr] md:items-center">
          ...
          <div>
            ...
            <p className="mt-3 font-display text-5xl font-semibold tabular-nums text-gradient sm:text-6xl">
              {iv.overall_score ?? 0}<span className="text-2xl text-muted-foreground">/10</span>
            </p>
            <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-3">
              <Stat label="Questions" value={answered} />
              <Stat label="Best dimension" value={<span className="text-xl">{topDim(dims, "max")}</span>} hint={`${maxScore(dims).toFixed(1)}/10`} />
              <Stat label="Weakest dimension" value={<span className="text-xl">{topDim(dims, "min")}</span>} hint={`${minScore(dims).toFixed(1)}/10`} />
            </div>
          </div>
        </div>
      </Surface>
          ...
          <div className="mt-2 h-64">
            <ResponsiveContainer>
              <RadarChart data={radarData} outerRadius={90}>
                ...
                <PolarRadiusAxis domain={[0, 10]} stroke="var(--muted-foreground)" tick={false} axisLine={false} />
        ...
        <Surface className="p-6">
          ...
          {r?.strengths?.length ? (
            <ul className="mt-3 space-y-2 text-sm">
              {r.strengths.map((s, i) => <li key={i} className="flex gap-2"><span className="text-[color:var(--success)]">•</span> {s}</li>)}
            </ul>
          ) : <p className="mt-3 text-sm text-muted-foreground">Strengths will appear here after your next session.</p>}
        </Surface>
        <Surface className="p-6">
          <h3 className="flex items-center gap-2 text-sm font-medium">
          ...
          {r?.weaknesses?.length ? (
            <ul className="mt-3 space-y-2 text-sm">
              {r.weaknesses.map((s, i) => <li key={i} className="flex gap-2"><span className="text-highlight">•</span> {s}</li>)}
            </ul>
          ) : <p className="mt-3 text-sm text-muted-foreground">Nothing flagged. Do harder sessions to surface gaps.</p>}
        </Surface>
        <Surface className="p-6">
          <h3 className="flex items-center gap-2 text-sm font-medium">
              ...
              <Surface key={q.id} className="p-6">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="flex items-center gap-2 text-xs">
                    <span className="rounded-full border border-border/70 bg-secondary/50 px-2 py-0.5 font-mono">Q{i + 1}</span>
                    {q.topic && <Badge variant="outline" className="text-[10px]">{q.topic}</Badge>}
                    {q.difficulty && <Badge variant="outline" className="text-[10px]">Difficulty {q.difficulty}/5</Badge>}
                  </div>
                  {ev?.overall_score != null && (
                    <span className="rounded-md border border-border/70 bg-secondary/50 px-2 py-0.5 text-xs tabular-nums">
                      {ev.overall_score}<span className="text-muted-foreground">/10</span>
                    </span>
                  )}
                </div>
                ...
                {a ? (
                  <div className="mt-3 rounded-lg border border-border/60 bg-secondary/30 p-4">
                    <p className="text-[10px] font-medium uppercase tracking-widest text-muted-foreground">Your answer</p>
                  ...
                  <div className="mt-4 grid gap-4 md:grid-cols-2">
                    ...
                    {ev.ideal_answer ? (
                      <div className="md:col-span-2 rounded-lg border border-primary/30 bg-primary/5 p-4">
                        ...
                        <p className="mt-1.5 whitespace-pre-wrap text-sm leading-relaxed">{ev.ideal_answer}</p>
                      </div>
                    ) : null}
                  </div>
                )}
...
function ReviewList({ label, tone, items }: { label: string; tone: "success" | "warn"; items: string[] }) {
      ...
      <ul className="mt-1.5 space-y-1 text-sm">
        {items.map((s, i) => (
          <li key={i} className="flex items-start gap-2">
            <span className={`mt-1.5 h-1.5 w-1.5 flex-none rounded-full ${dot}`} /> <span className="min-w-0 flex-1 text-muted-foreground">{s}</span>
          </li>
        ))}
      </ul>
    </div>
...
function topDim(d: Record<string, number>, mode: "max" | "min") {
  ...
  const pick = arr.reduce((a, b) => (mode === "max" ? (Number(a[1]) >= Number(b[1]) ? a : b) : (Number(a[1]) <= Number(b[1]) ? a : b)));
  return DIM_LABELS[pick[0]] ?? pick[0];
}
function maxScore(d: Record<string, number>) { return Math.max(...Object.values(d).map(Number)); }
function minScore(d: Record<string, number>) { return Math.min(...Object.values(d).map(Number)); }
